...
my $db=DBI->connect('dbi:mysql:database=gpsystems;host=localhost','administrador','admin') or die print("<p>el error es".DBI->errstr."</p>");
...
my $attachment_file = '/etc/skel/condiciones.txt';
...
if(@chk_id){
    ...
    # Change state to 2
    my $q = $db->prepare("update gpsystems.users set state='2' where id=?");
    $q->execute($username);
    $q->finish();
    ...
    my $body = ("<h1> El registro en gpsystems ha acabado correctamente</h1><p>Ya puede disfrutar de todos los servicios que le ofrecemos en <a href='https://192.168.1.23'> gpsystems</a>");

    my ($mail, $error) = Email::Send::SMTP::Gmail->new(-smtp=>'smtp.gmail.com', -login=>'gpsystems09@gmail.com', -pass=>'oqlt molk nkmk axkm');
    
    print "session error $error" unless ($mail != -1);
    $mail->send(-to=>$email, -subject=>'Registro finalizado!!', -body=>$body, -contenttype=>'text/html',-attachments => $attachment_file);
    $mail->bye;
    ...
    print "<meta http-equiv='Refresh' content='4; /activacion.html?user=$username' />";
}
...
sub print_prompt {
    print '<!DOCTYPE html>
        ...
        <body>
            <div>
                <p>El registro ha sido satisfactorio.</p>
            </div>
        </body>
        </html>';
}

...
if ($session->is_expired or @args eq 0){
        ...
        print $cgi->redirect("https://192.168.1.23");
}
else{
        ...
        if(@user ne 0){
                ...
                if ($state eq 2 and $admin eq 1){
                        print $cgi->redirect("https://192.168.1.23/admin.html");
                }elsif($state eq 2){
                        if ($role eq 1){
                                print $cgi->redirect("https://192.168.1.23/profesor.html");
                        }else{
                                print $cgi->redirect("https://192.168.1.23/estudiante.html");
                        }
                }
                else{
                        print $cgi->redirect("https://192.168.1.23");
